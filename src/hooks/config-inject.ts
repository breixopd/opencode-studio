...
const AGENT_DEFS: AgentDef[] = [
  {
    ...
    guidance: "Explore read-only. Start with glob to understand structure, then symbols outline, then index semantic for deeper understanding. Never make edits. Good: glob → symbols → index semantic. Bad: read random files.",
    edit: "deny",
    bash: "deny",
  },
  {
    ...
    guidance: "Research with studio_search (use scrape:true for top hits). Prefer primary sources (official docs, RFCs, source code). Cite URLs. Studio_fetch for specific pages, studio_crawl for multi-page docs. Good: studio_search then studio_fetch primary docs with cited URLs. Bad: hallucinate APIs or rely on secondary blog spam.",
    edit: "deny",
    bash: "deny",
  },
  {
  ...
  },
  {
    ...
    tools: ["studio_verify"],
    guidance: "Run studio_verify. Report failures with file:line in the output. If verify fails persistently (3x), suggest snapshot+rollback. Do not fix issues yourself — report them for @studio-implement. Good: run studio_verify, report file:line failures, suggest rollback after 3x. Bad: try to fix failing issues yourself.",
    edit: "deny",
    bash: "allow",
  },
  {
    ...
    guidance: "Remote dev via studio tools. Sync is automatic on session start. Use studio_remote to run commands on remote hosts. Check studio_status for overall health. Good: check studio_status, run remote commands via studio_remote. Bad: assume sync is manual or skip health checks.",
    edit: "allow",
    bash: "allow",
  },
]
